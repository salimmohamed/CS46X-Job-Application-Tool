...
After the upload, you land on the Candidate Details form. Fields the parser couldn't find are highlighted in yellow — those need your attention. White fields were filled in automatically but you can still edit them. Once everything looks right, hit **"Save Profile"** at the bottom. Green banner means it worked; red means something went wrong (usually the backend isn't running).
...
1. Run `npm run build` inside `frontend/` — this generates the files the extension needs.
